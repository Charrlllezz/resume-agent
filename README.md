...
**Two-column resumes come back two-column.** The geometry is measured, not
guessed: a sidebar panel is a large filled rectangle down one edge of the
page, and the file states its position, its width and its colour. Where there
is no panel, the column split is inferred from where the text sits — weighted
by how much text, so a right-floated date column is not mistaken for a sidebar.

Which sections belong in the sidebar is the judged part, constrained to the
sections that actually exist. Text colour on the panel is derived from how
dark it is.

The tool's own output reads back with the same geometry it detected, which is
the check that the loop actually closed.
